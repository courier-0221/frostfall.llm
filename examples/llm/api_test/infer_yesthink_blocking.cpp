// 非流式（blocking）接口端到端测试（开启深度思考）
//
// 全部用例统一：stream=false, enable_thinking=true
// 覆盖：
//   1. 简单单轮
//   2. Tool Calling 单轮
//   3. 多轮（含 tool 结果回填）
//
// 与 infer_yesthink_stream.cpp 一一对应，区别仅在于 stream=false、
// Infer 不传回调、一次性拿完整 LlmResponse（thinking 累积在 resp.thinking）。
...
// ============================================================
// Test 1: 简单单轮 — 非流式 + thinking
// ============================================================
static void test_simple_single_turn(InferenceEngine& engine) {
    std::cout << "\n========================================\n"
              << "[Blocking Test 1] 简单单轮 (stream=false, thinking=true)\n"
              << "========================================\n\n";

    LlmRequest req;
    req.msg_id          = "blocking-think-0001";
...
// ============================================================
// Test 2: Tool Calling — 非流式 + thinking
// ============================================================
static void test_tool_calling(InferenceEngine& engine) {
    std::cout << "\n========================================\n"
              << "[Blocking Test 2] Tool Calling (stream=false, thinking=true)\n"
              << "========================================\n\n";

    LlmRequest req;
    req.msg_id          = "blocking-think-0002";
    req.enable_thinking = true;
    req.stream          = false;
    req.messages = {
...
// ============================================================
// Test 3: 多轮（含 tool 结果回填）— 非流式 + thinking
// ============================================================
static void test_multi_turn(InferenceEngine& engine) {
    std::cout << "\n========================================\n"
              << "[Blocking Test 3] 多轮 + Tool 结果回填 (stream=false, thinking=true)\n"
              << "========================================\n\n";

    // ---------- Round 1: 触发 tool call ----------
    LlmRequest round1;
    round1.msg_id          = "blocking-think-0003-1";
    round1.enable_thinking = true;
    round1.stream          = false;
    round1.messages = {
        Message{Role::SYSTEM, car_system_prompt(), "", {}},
    ...
    LlmRequest round2;
    round2.msg_id          = "blocking-think-0003-2";
    round2.enable_thinking = true;
    round2.stream          = false;
...
int main(int argc, char* argv[]) {
    ...
    if (!parse_args(argc, argv, opt) || opt.help) {
        print_usage(argv[0], "All tests use: stream=false, enable_thinking=true");
        return opt.help ? 0 : 1;
    }
    ...
    return runner.summary("blocking, thinking");
}
