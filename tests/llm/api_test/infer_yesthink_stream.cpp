// 流式接口端到端测试（开启深度思考）
//
// 全部用例统一：stream=true, enable_thinking=true
// 覆盖：
//   1. 简单单轮
//   2. Tool Calling 单轮
//   3. 多轮（含 tool 结果回填）
//
// 与 infer_nothink_stream.cpp 一一对应，区别仅在于 enable_thinking=true：
// 模型会先输出 <think>..</think> 思考段（走 thinking 帧），再输出正文 / tool_call。
...
// ============================================================
// Test 1: 简单单轮 — 流式 + thinking
// ============================================================
static void test_simple_single_turn(InferenceEngine& engine) {
    std::cout << "\n========================================\n"
              << "[Stream Test 1] 简单单轮 (stream=true, thinking=true)\n"
              << "========================================\n\n";

    LlmRequest req;
    req.msg_id          = "stream-think-0001";
...
// ============================================================
// Test 2: Tool Calling — 流式 + thinking
// ============================================================
static void test_tool_calling(InferenceEngine& engine) {
    std::cout << "\n========================================\n"
              << "[Stream Test 2] Tool Calling (stream=true, thinking=true)\n"
              << "========================================\n\n";

    LlmRequest req;
    req.msg_id          = "stream-think-0002";
    req.enable_thinking = true;
    req.stream          = true;
    req.messages = {
...
// ============================================================
// Test 3: 多轮（含 tool 结果回填）— 流式 + thinking
// ============================================================
static void test_multi_turn(InferenceEngine& engine) {
    std::cout << "\n========================================\n"
              << "[Stream Test 3] 多轮 + Tool 结果回填 (stream=true, thinking=true)\n"
              << "========================================\n\n";

    // ---------- Round 1: 触发 tool call ----------
    LlmRequest round1;
    round1.msg_id          = "stream-think-0003-1";
    ...
    LlmRequest round2;
    round2.msg_id          = "stream-think-0003-2";
...
int main(int argc, char* argv[]) {
    ...
    if (!parse_args(argc, argv, opt) || opt.help) {
        print_usage(argv[0], "All tests use: stream=true, enable_thinking=true");
        return opt.help ? 0 : 1;
    }
    ...
    return runner.summary("stream, thinking");
}
